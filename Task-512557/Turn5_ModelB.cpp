...
public:
    ...
    std::string get(const std::string& key, const std::string& threadName, const std::chrono::milliseconds& timeout = std::chrono::milliseconds::max()) {
        ...
        // Wait until there are no waiting writers or timeout occurs
        auto result = cv.wait_for(lock, timeout, [this]() { return waitingWriters == 0; });

        if (result == std::cv_status::timeout) {
            std::ostringstream logStream;
            logStream << "[" << threadName << "] timed out while waiting for read access.";
            std::cout << logStream.str() << std::endl;
            return ""; // Return empty string or handle timeout as per your application
        }
    ...
    void update(const std::string& key, const std::string& value, const std::string& threadName, const std::chrono::milliseconds& timeout = std::chrono::milliseconds::max()) {
        auto startTime = std::chrono::steady_clock::now(); // Start timing
        std::unique_lock<std::shared_mutex> lock(mtx);

        ++waitingWriters; // Increase count of waiting writers

        // Wait until there are no active readers or timeout occurs
        auto result = cv.wait_for(lock, timeout, [this]() { return activeReaders == 0; });

        if (result == std::cv_status::timeout) {
            std::ostringstream logStream;
            logStream << "[" << threadName << "] timed out while waiting for write access.";
            std::cout << logStream.str() << std::endl;
            return; // Handle timeout as per your application
        }
