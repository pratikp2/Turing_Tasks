...
void metropolisHastings(int numAgents, int numIterations, double temperature) {
  ...
  for (int iteration = 0; iteration < numIterations; iteration++) {
    ...
    int newPosition =
        (agentPositions[agentIndex] + 1) %
        numAgents; // Move the agent to the next position (wrapping around)

    double deltaE =
        0.0; // Calculate the energy change (for simplicity, we'll use 0 here)
...
int main() {
  const int numAgents =
      100000; // Assuming a large number of agents for edge cases
  const int numIterations = 100000;
  const double temperature = 1.0;

  // Dynamically allocate memory for agent positions
  std::vector<int> agentPositions(numAgents, 0);
