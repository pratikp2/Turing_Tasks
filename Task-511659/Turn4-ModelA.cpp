...
struct LogList {
    ...
    // Function to analyze trends in user behavior
    void analyzeTrends() {
        std::map<std::string, int> actionCounts;
        const int peakHour = 24; // For daily trends

        // Count occurrences of each action
        LogEntry* current = head;
        while (current) {
            std::string action(current->message);
            size_t pos = action.find(" | "); // Find the separator
            if (pos != std::string::npos) {
                action = action.substr(11, pos - 11); // Extract action (first 11 characters)
                actionCounts[action]++;
            }
            current = current->next;
        }
        ...
        std::cout << "Engagement Trends:" << std::endl;
...
void analyzeUserBehavior() {
    ...
    // After all actions, analyze trends
    logs.analyzeTrends();
