...
void TestMyFunction(const std::string &input,
                    const std::string &expectedOutput) {
  std::string output = MyFunction(input);
  if (output == expectedOutput) {
    std::cout << "Test Passed: Input: " << input
              << ", Expected: " << expectedOutput << ", Output: " << output
              << std::endl;
  } else {
...
int main() {
  ...
  // Test 2: Checking if the function correctly uppercases a single lowercase
  // letter
  TestMyFunction("a", "A");

  // Test 3: Checking if the function correctly uppercases a multiple lowercase
  // letters
  TestMyFunction("hello", "HELLO");

  // Test 4: Checking if the function correctly uppercases a mix of lowercase
