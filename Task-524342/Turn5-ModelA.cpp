...
public:
    static void log(const std::string& message) {
        std::cout << message << std::endl; // For simplicity, we log to the console
    }
};
...
public:
    ...
    DynamicArray(size_t size)
        ...
        Logger::log("Allocated DynamicArray of size " + std::to_string(size_));
    }

    // Destructor that deallocates memory
    ~DynamicArray() {
        Logger::log("Deallocating DynamicArray of size " + std::to_string(size_));
    ...
    DynamicArray(DynamicArray&& other) noexcept
        ...
        other.data_ = nullptr;
        Logger::log("Moved DynamicArray of size " + std::to_string(size_));
    }

    // Move assignment operator
    DynamicArray& operator=(DynamicArray&& other) noexcept {
        if (this != &other) {
            Logger::log("Moving assignment of DynamicArray from size " +
                         std::to_string(other.size_) + " to size " + std::to_string(size_));
    ...
    void resize(size_t newSize) {
        ...
        try {
            ...
            Logger::log("Reallocating DynamicArray from size " + std::to_string(size_) +
                         " to size " + std::to_string(newSize));
            ...
            Logger::log("Memory allocation failed during reallocation.");
...
public:
    void allocate(size_t size) {
        try {
            ...
            Logger::log("Exception during allocation: " + std::string(e.what()));
        }
    }
