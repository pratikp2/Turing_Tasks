...
int main() {
    ...
    // Asynchronous execution with std::thread (for comparison)
    std::cout << "\nAsynchronous execution with std::thread:\n";
    {
        auto start_time = std::chrono::high_resolution_clock::now();
        std::thread thread1(calculateSquare, 1, std::ref(result_1));
        std::thread thread2(calculateSquare, 2, std::ref(result_2));
